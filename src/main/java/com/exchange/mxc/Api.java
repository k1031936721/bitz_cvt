...
public class Api {
    ...
    public void getMarketDepth() throws Exception {
        URL url = new URL(MARKET_DEPTH + "?market=CVT_USDT&depth=20");
        URLConnection urlConnection = url.openConnection();
        urlConnection.setRequestProperty("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.96 Safari/537.36");
        InputStream inputStream = urlConnection.getInputStream();
        BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(inputStream));
    ...
    public String getMarketDepth() throws Exception {
        ...
        bufferedReader.lines().forEach(e -> sbf.append(e));
        Thread.sleep(1000);
        return sbf.toString();
    }
    ...
    public String getTicket(String market) throws Exception {
//        String market = "CVT_USDT";
        URL url = new URL(TICKET + "?market=" + market);
        URLConnection urlConnection = url.openConnection();
        urlConnection.setRequestProperty("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.96 Safari/537.36");
